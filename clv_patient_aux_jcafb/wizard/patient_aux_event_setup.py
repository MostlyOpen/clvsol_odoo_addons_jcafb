...
class PatientAuxEventSetUp(models.TransientModel):
    _description = 'Patient (Aux) Event Setup'
    _name = 'clv.patient_aux.event_setup'

    def _default_patient_aux_ids(self):
        return self._context.get('active_ids')
    patient_aux_ids = fields.Many2many(
        comodel_name='clv.patient_aux',
        relation='clv_patient_aux_event_setup_rel',
        string='Patients (Aux)',
    ...
    event_ids = fields.Many2many(
        ...
        relation='clv_patient_aux_event_setup_event_rel',
        string='Events'
    )

    document_type_ids = fields.Many2many(
        comodel_name='clv.document.type',
        relation='clv_patient_aux_event_setup_document_type_rel',
    ...
    def do_patient_aux_event_setup(self):
        self.ensure_one()
