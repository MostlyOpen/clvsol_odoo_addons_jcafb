...
class PatientAssociateToSet(models.TransientModel):
    _description = 'Patient Associate to Set'
    _name = 'clv.patient.associate_to_set'

    def _default_patient_ids(self):
        return self._context.get('active_ids')
    patient_ids = fields.Many2many(
        comodel_name='clv.patient',
        relation='clv_patient_associate_to_set_rel',
        string='Patients',
    ...
    def do_patient_associate_to_set(self):
        self.ensure_one()
        ...
        for patient in self.patient_ids:
            ...
            if set_element.id is False:
                ...
                values['ref_id'] = patient._name + ',' + str(patient.id)
                _logger.info(u'%s %s %s', '>>>>>>>>>>', 'values:', values)
