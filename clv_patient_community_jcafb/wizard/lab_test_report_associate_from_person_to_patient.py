...
class LabTestReportAssociateFromPersonToPatient(models.TransientModel):
    _description = 'Lab Test Report Associate from Person to Patient'
    _name = 'clv.lab_test.report.associate_from_person_to_patient'

    def _default_lab_test_report_ids(self):
        return self._context.get('active_ids')
    lab_test_report_ids = fields.Many2many(
        comodel_name='clv.lab_test.report',
        relation='clv_lab_test_report_associate_from_person_to_patient_rel',
        string='Lab Test Reports',
        default=_default_lab_test_report_ids
    )

    def do_lab_test_report_associate_from_person_to_patient(self):
        self.ensure_one()
        ...
        for lab_test_report in self.lab_test_report_ids:
            ...
            if (lab_test_report.ref_model == 'clv.person') and (lab_test_report.ref_id.is_patient is True):

                related_patient = lab_test_report.ref_id.patient_ids
